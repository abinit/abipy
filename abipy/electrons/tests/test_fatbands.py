...
class TestElectronFatbands(AbipyTest):
    def test_MgB2_fatbands(self):
        ...
        if self.has_matplotlib():
            ...
            assert fbnc_kpath.plot_fatbands_siteview(
                e0=None, view="inequivalent", fact=2.0, bdist=[1, 2, 3], show=False
            )
        ...
        if self.has_matplotlib():
            ...
            assert fbnc_kpath.plot_fatbands_with_pjdos(
                pjdosfile=fbnc_kmesh, view="lview", tight_layout=True, show=False
            )
        ...
        if self.has_matplotlib():
            for combined_spins, stacked in itertools.product([True, False], [True, False]):
                assert fbnc_kmesh.plot_pjdos_typeview(
                    xlims=elims, combined_spins=combined_spins, stacked=stacked, tight_layout=True, show=False
                )
            ...
            assert fbnc_kpath.plot_fatbands_with_pjdos(
                pjdosfile=fbnc_kmesh, ylims=elims, lmax=lmax, view="type", tight_layout=True, show=False
            )
            assert fbnc_kpath.plot_fatbands_with_pjdos(
                pjdosfile=fbnc_kmesh, ylims=elims, lmax=lmax, view="lview", tight_layout=True, show=False
            )
            ...
            assert (
                fbnc_kpath.plot_fatbands_mview(
                    iatom=0, lmax=2, fact=1.5, ylims=[-1.5, +1.0], blist=list(range(4, 10)), show=False
                )
                is not None
            )
