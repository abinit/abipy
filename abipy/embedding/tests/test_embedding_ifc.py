...
class Embedding_ifcTest(AbipyTest):
    ...
    def test_embedding_interstitial(self):
        ...
        qpts = kmesh_from_mpdivs(mpdivs=[3, 3, 3], shifts=[0, 0, 0], order="unit_cell")
        ddb_unit_333 = ddb_pristine.anaget_interpolated_ddb(
            qpt_list=qpts,
        )

        ph_pristine = ddb_ucell_to_phonopy_supercell(
            unit_ddb=ddb_unit_333,
            nac=False,
        )
        ...
        ph_emb = Embedded_phonons.from_phonopy_instances(
            ...
            main_defect_coords_in_defect=main_defect_coords_in_defect,
            vacancies_list=[31],
            interstitial_list=[["C", [4.2885, 3.5737, 1.7869]], ["C", [2.8590, 3.5737, 1.7869]]],
            verbose=1,
            cut_off_mode="auto",
            factor_ifc=1,
            asr=True,
