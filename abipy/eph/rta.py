...
def edos_infos(edos_intmeth, edos_broad) -> str:
    ...
    if edos_intmeth == 1:
        s = "%s with broadening: %.1f (meV)" % edos_broad * abu.Ha_to_meV
...
class RtaFile(AbinitNcFile, Has_Structure, Has_ElectronBands, NotebookWriter):
    ...
    def plot_tau_isoe(self, ax_list=None, colormap="jet", fontsize=8, **kwargs) -> Figure:
        ...
        ax_list, fig, plt = get_axarray_fig_plt(
            ax_list, nrows=self.nrta, ncols=1, sharex=True, sharey=True, squeeze=False
        )
        ...
        for irta, ax in enumerate(ax_list):
            for spin in range(self.nsppol):
                ...
                for it, temp in enumerate(self.tmesh):
                    ...
                    ys = spin_sign * tau_dos[irta, spin, it] * abu.Time_Sec * 1e15
                    ax.plot(
                        self.edos_mesh_eV, ys, c=cmap(it / self.ntemp), label="T = %dK" % temp if spin == 0 else None
                    )
    ...
    def plot_vvtau_dos(self, component="xx", spin=0, ax=None, colormap="jet", fontsize=8, **kwargs) -> Figure:
        ...
        for irta in range(self.nrta):
            ...
            for itemp, temp in enumerate(self.tmesh):
                ...
                label = "T = %dK" % temp
                if itemp == 0:
                    label = "%s (%s)" % (label, irta2s(irta))
                if irta == 0 and itemp > 0:
                    label = None
    ...
    @add_fig_kwargs
    def plot_mobility(
        self, eh=0, irta=0, component="xx", spin=0, ax=None, colormap="jet", fontsize=8, yscale="log", **kwargs
    ) -> Figure:
        """
