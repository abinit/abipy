...
class CumulantQpTempState(QpTempState):
    ...
    def set_energies(self, wmesh, vals_wr, vals_e0ks, ntemp, nwr):
        ...
        for it in range(ntemp):
            ...
            idx = np.argwhere(
                np.diff(np.sign(vals_wr[it, nwr // 4 : 3 * nwr // 4].real - wmesh[nwr // 4 : 3 * nwr // 4] + self.e0))
            ).flatten()
...
class CumulantPhReader(SigmaPhReader):
    ...
    def read_cumulant_skb(self, spin, kpoint, band) -> CumulantSelfEnergy:
        ...
        return CumulantSelfEnergy(
            wmesh, qp, gw_vals, spfunccumul_wr, time_mesh=time_mesh, ct_vals=ct_vals, gt_vals=gt_vals
        )
...
class CumulantSelfEnergy(EphSelfEnergy):
    ...
    def calculate_gw_from_sigeph_skb(cls, sigeph, time_tol=1e-4) -> CumulantQpTempState:
        ...
        dw = np.zeros(ntemp)
        ze0 = 0.0
        qp = CumulantQpTempState(
            spin=sigeph.spin,
            kpoint=sigeph.kpoint,
            band=sigeph.band,
            tmesh=sigeph.tmesh,
            e0=sigeph.qp.e0,
            qpe=qpe,
