...
class CumEPhFile(AbinitNcFile, Has_Structure, Has_ElectronBands, NotebookWriter):
    """
    This file contains the Spectral Function using the cumulant expansion and Fan-Migdal Debye-Waller self-energy, the |ElectronBands| on the k-mesh.
    Provides methods to analyze and plot results.
    ...
    .. code-block:: python
        ...
        with CumEPhFile("out_EPH_CUMULANT.nc", SigmaEphFile) as ncfile:
            print(ncfile)
            ncfile.ebands.plot()

    .. rubric:: Inheritance Diagram
    .. inheritance-diagram:: CumEPhFile
    """
    # Markers used for up/down bands.
    marker_spin = {0: "^", 1: "v"}
    color_spin = {0: "k", 1: "r"}
    ...
    @classmethod
    def from_file(cls, filepath, sigmaobj):
        """Initialize the object from a netcdf file."""
        return cls(filepath)

    def __init__(self, filepath, sigmaobj):
        super().__init__(filepath)
        self.reader = rc = CumulantPhReader(filepath, sigmaobj)
        r = sigmaobj
        # Get important dimensions.
        self.nkcalc = r.nkcalc
        self.ntemp = r.ntemp
        ...
        self.symsigma = r.symsigma
        # 4 for FAN+DW, -4 for Fan Imaginary part
        #self.eph_task == r.eph_task
        self.imag_only = r.imag_only
        # TODO zcut?
        self.zcut = r.zcut
        self.nbsum = r.nbsum

        self.bstart_sk = r.bstart_sk
        self.nbcalc_sk = r.nbcalc_sk
        self.bstop_sk = r.bstop_sk
    ...
    def get_gw_skb(self, spin, kpoint, band, sigmaobj = None):
        """"Return e-ph self-energy for the given (spin, kpoint, band)."""
        #return self.reader.read_gw_skb(spin, kpoint, band)
        return self.reader.read_gw_skb(spin, kpoint, band, sigmaobj)
...
class CumulantPhReader(BaseEphReader):
    """
    Reads data from file and constructs objects.
    ...
    """
    def __init__(self, path, sigmaobj):
        super().__init__(path)
        r = sigmaobj
    ...
    def read_gw_skb(self, spin, kpoint, band, sigmaobj = None):
        """
        Returns the Green's Function in frequency domain for the given (spin, k-point, band).
        ...
        if sigmaobj is not None:
            #spin_sig, ikc_sig, ib_sig, kpoint_sig = sigmaobj.get_sigma_skb_kpoint(spin, kpoint, band)
            spin_sig = sigmaobj.spin
            kpoint_sig = sigmaobj.kpoint
            band_sig = sigmaobj.band
            print(spin, ikc, band, kpoint)
            print(spin_sig, band_sig, kpoint_sig)
            if spin != spin_sig or band != band_sig or kpoint != kpoint_sig:
                raise ValueError("Spin, kpoint or band not the same as sigma object")
            wmesh = sigmaobj.wmesh
            qp = sigmaobj.qp
        else:
            try:
                wmesh = self.read_variable("wrmesh_b")[spin, ikc, ib, :] * abu.Ha_eV
            except:
                raise ValueError("wrmesh_b not present in NETCDF file")
            try:
                # Read QP data. Note band instead of ib index.
                qp = self.read_qp(spin, ikc, band)
            except:
                raise ValueError("qp variables not present in NETCDF file")
        ...
        # complex(dpc) :: gw_vals(complex, nwr, ntemp, max_nbcalc, nkcalc, nsppol)
        # G(omega, kT, band) for given (k, spin)
        gw_vals = self.read_variable("gw_vals")[spin, ikc, ib, :, :, :]# * abu.Ha_eV
        gw_vals = gw_vals[:, :, 0] + 1j * gw_vals [:, :, 1]

        # Spectral function
        spfunc_wr = -1.0 * gw_vals.imag / np.pi


        return CumSpectralFunction(wmesh, qp, gw_vals, spfunc_wr)
...
class CumSpectralFunction(object):
    r"""
    Electron spectral function due to phonon interaction :math:`A_{nk}(\omega,T)`
    Actually this object stores the diagonal matrix elements in the KS basis set.
    """
    # Symbols used in matplotlib plots.
    latex_symbol = dict(
    ...
    def __init__(self, wmesh, qp, gw_vals, spfunc_wr):
        """
        Args:
            wmesh: Frequency mesh in eV.
            qp: :class:`QpTempState` instance.
            gw_vals: complex [ntemp, nwr] Green's Function in frequency domain
        """
        self.qp = qp
        self.spin, self.kpoint, self.band = qp.spin, qp.kpoint, qp.band
        self.wmesh, self.tmesh = wmesh, qp.tmesh
        ...
        assert self.gw_vals.shape == (self.ntemp, self.nwr)
    ...
    def _get_ys_itemp(self, what, itemp, select_frohl=False):
        ...
        """
        return dict(
                re=self.gw_vals[itemp].real,
                im=self.gw_vals[itemp].imag,
                spfunc=self.spfunc_wr[itemp],
        )[what]
    ...
    def plot_tdep(self, itemps="all", zero_energy="e0", colormap="jet", ax_list=None,
        ...
        Args:
            ...
            xlims: Set the data limits for the x-axis. Accept tuple e.g. ``(left, right)``
                or scalar e.g. ``left``. If left (right) is None, default values are used.
            xlims: Set the data limits for the y-axis. Accept tuple e.g. ``(bottom, top)``
                or scalar e.g. ``bottom``. If bottom (top) is None, default values are used.
            fontsize: legend and label fontsize.
            kwargs: Keyword arguments passed to ax.plot
    ...
    def plot_qpsolution(self, itemp=0, with_int_aw=True, ax_list=None, xlims=None, fontsize=8, **kwargs):
        ...
        # Plot G(w)
        ax0.grid(True)
        ax0.plot(xs, self.vals_wr[itemp].real, label=r"$\Re(G)$")
        ax0.plot(xs, self.vals_wr[itemp].imag, ls="--", label=r"$\Im(G)$")
        ax0.plot(xs, self.wmesh - self.qp.e0, color="b", lw=1,
                 ls=linestyles["dashed"], label=r"$\omega - \epsilon^0$")
