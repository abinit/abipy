...
class SigEPhFile(AbinitNcFile, Has_Structure, Has_ElectronBands, NotebookWriter):
    ...
    def get_lifetimes_boltztrap(self, basename, workdir=None):
        ...
        with open(basename+'_BLZTRP.structure','w') as f:
            ...
            f.write(fmt3%tuple(struct.lattice.matrix[0]*Ang_Bohr))
            f.write(fmt3%tuple(struct.lattice.matrix[1]*Ang_Bohr))
            f.write(fmt3%tuple(struct.lattice.matrix[2]*Ang_Bohr))
            f.write("%d\n"%len(struct))
            for atom in struct:
