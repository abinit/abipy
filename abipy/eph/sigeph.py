...
class SigEPhFile(AbinitNcFile, Has_Structure, Has_ElectronBands, NotebookWriter):
    ...
    def get_lifetimes_boltztrap(self,basename):
        ...
        for itemp in range(ntemp):
            T = self.tmesh[itemp]
            filename_tau = basename+'_%dK.tau'%T
            with open(filename_tau,'w') as ftau:
                ftau.write('BoltzTraP tau_k file generated by abipy for T=   %12.6lf\n'%T)
                ftau.write('%5d %5d %20.10e ! nk, nspin : lifetimes below in s \n'%(nkpt,nspn,fermie))
        ...
        with open(filename_ene,'w') as fene:
            fene.write('BoltzTraP eigen-energies file generated by abipy.\n') 
            fene.write('%5d %5d %20.10e ! nk, nspin, Fermi level (Ry) : energies below in Ry \n'%(nkpt,nspn,fermie))
            for ispin in range(nspn):
                for ik in range(nkpt):
                    kpt = self.kpoints[ik]
                    fmt = '%20.10e '*3+'%d !kpt nband\n'%(nband_stop-nband_start)
