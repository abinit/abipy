...
class TestFunction1D(AbipyTest):
    ...
    def test_base(self):
        ...
        assert cosf / 2.0 == Function1D(cosf.mesh, cosf.values / 2.0)

        twof = Function1D(cosf.mesh, 2 * np.ones(len(cosf)))
        assert cosf / 2.0 == cosf / twof
        assert cosf * 2.0 == cosf * twof
        assert 1 - cosf**2 == sinf**2
        self.assert_almost_equal((1 - cosf).integral()[-1], 2 * np.pi)
        self.assert_almost_equal(sinf.l1_norm, 4.0, decimal=4)
        ...
        assert np.abs((sinf.finite_diff(order=4, acc=4) - sinf).max) < 1.0e-5

        assert not sinf.iscomplexobj
        assert eix.iscomplexobj
        self.assert_almost_equal(cosf.max, 1.0)
        self.assert_almost_equal(abs(sinf).min, 0)
        self.assert_almost_equal(eix.max, 1.0)
        self.assert_almost_equal(eix.min, 1.0)
