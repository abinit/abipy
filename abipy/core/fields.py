...
class _Field(Has_Structure):
    ...
    Data is represented on a homogeneous real-space mesh.
    ...
    def get_planar_average(self, direction: str, spin: int) -> tuple[np.ndarray]:
        """
        Compute the planar average of the field in real space along surfaces perpendicular
        to a given lattice vector.

        Args:
            direction:

        Return: (dists, averages)
            where dists is a [npts] array with the distance in Ang and averages is a (nspden, npts)
            with the planar average.
        """
        abc = self.structure.lattice.abc

        # datar has shape [nspden, nx, ny, nz]
        if direction == "a":
            fact = 1.0 / (self.ny * self.nz)
            step = abc[0] / self.nx
            # [nx, nspden, ny, nz]
            values = self.datar.transpose(1, 0, 2, 3).copy()
        elif direction == "b":
            fact = 1.0 / (self.nx * self.nz)
            step = abc[1] / self.ny
            # [ny, nspden, nx, nz]
            values = self.datar.transpose(2, 0, 1, 3).copy()
        elif direction == "c":
            fact = 1.0 / (self.nx * self.ny)
            step = abc[2] / self.nz
            # [nz, nspden, nx, ny]
            values = self.datar.transpose(3, 0, 1, 2).copy()
        else:
            raise ValueError(f"Invalid {direction=}")

        npts = values.shape[0]
        dists, averages = np.zeros(npts), np.zeros(npts)

        for ip in range(npts):
            dists[ip] = ip * step
            if self.nspinor == 1:
                averages[ip] = values[ip, spin].sum()
            else:
                averages[ip] = values[ip, spin].sum()

        return dists, fact * averages
    ...
    def plot_planar_average(self, direction, spin=0, ax=None, fontsize=12, **kwargs) -> Figure:
        ...
        ax.plot(dists, averages, color="green", ls="-")

        set_grid_legend(ax, fontsize, xlabel=r"Distance ($\AA$)", ylabel=self.latex_label)
        ax.set_title(f"Planar average perpendicular to direction {direction}", fontsize=fontsize)
...
class Density(_DensityField):
    ...
    def ae_core_density_on_mesh(cls, valence_density, structure, rhoc,
        ...
        Args:
            ...
            nelec: if given a check will be performed to verify that the integrated density will be within 1% error
...
class FieldReader(ETSF_Reader):
    ...
    def read_density(self, field_cls=Density) -> Density:
        """Read density data. Return `Density` object."""
        return self.read_denpot(varname=field_cls.netcdf_name, field_cls=field_cls)

    def read_vh(self, field_cls=VhartreePotential) -> VhartreePotential:
        """Read potential data. Return `VhartreePotential` object."""
        return self.read_denpot(varname=field_cls.netcdf_name, field_cls=field_cls)

    def read_vxc(self, field_cls=VxcPotential) -> VxcPotential:
        """Read potential data. Return `VxcPotential` object."""
        return self.read_denpot(varname=field_cls.netcdf_name, field_cls=field_cls)

    def read_vhxc(self, field_cls=VhxcPotential) -> VhxcPotential:
        """Read potential data. Return VhxcPotential object."""
        return self.read_denpot(varname=field_cls.netcdf_name, field_cls=field_cls)

    def read_vks(self, field_cls=VksPotential) -> VksPotential:
    ...
    def read_denpot(self, varname, field_cls):
        ...
        #################################################
        # Abinit conventions for GS densities/potentials
        #################################################
        ...
        # Get rid of fake last dimension if cplex == 1 or convert to complex array if cplex == 2.
        ...
        if dims.nspinor == 1:
            if dims.nspden == 1:
                ...
                if issubclass(field_cls, _DensityField):
                    # If Density-like: store rho_up, rho_down instead of rho_total, rho_up.
                    total = datar[0].copy()
        ...
        if issubclass(field_cls, _DensityField):
            ...
            raise TypeError(f"Don't know how to handle {field_cls=}")
