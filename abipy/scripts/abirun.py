...
def select_nids(flow: Flow, options) -> set:
    ...
    task_ids = [
        task.node_id
        for task in flow.select_tasks(nids=options.nids, wslice=options.wslice, task_class=options.task_class)
    ]
...
def flow_watch_status(flow: Flow, delay=5, nids=None, verbose=0, func_name="show_func"):
    ...
    """
    cprint(
        "Entering infinite loop (delay: %d s). Only changes are shown\nPress <CTRL+C> to exit" % delay,
        color="magenta",
        end="",
        flush=True,
    )
    ...
    try:
        while True:
            ...
            if tot_count == 1:
                ...
                if len(before_task2stat) == len(now_task2stat) and all(
                    now_task2stat[t] == before_task2stat[t] for t in now_task2stat
                ):
            ...
            if show_func is flow.show_status:
                flow.show_summary()
...
def get_parser(with_epilog=False):
    ...
    def parse_nids(s):
        ...
        try:
            ...
            s = duck.as_slice(s)
            if s.stop is None:
                raise argparse.ArgumentTypeError("stop must be specified")
    ...
    group.add_argument(
        ...
        type=parse_wslice,
        help=(
            "Select the list of works to analyze (python syntax for slices): "
    ...
    parser.add_argument(
        ...
        nargs="?",
        help=(
            "File or directory containing the ABINIT flow/work/task. "
    ...
    p_status.add_argument(
        "-s", "--summary", default=False, action="store_true", help="Print short version with status counters."
    )

    # Subparser for set_status command.
    p_set_status = subparsers.add_parser(
        "set_status",
        parents=[copts_parser, flow_selector_parser],
        help="Change the status of the task. WARNING: Option for developers!",
    )
    p_set_status.add_argument(
        "new_status", help="New value of status. Possible values: %s." % Status.all_status_strings()
    )
    ...
    p_restart = subparsers.add_parser(
        ...
        parents=[copts_parser, flow_selector_parser],
        help=(
            "Restart the tasks of the flow. By default, only the task whose status==Unconverged are restarted. "
    ...
    p_move = subparsers.add_parser(
        "move", parents=[copts_parser], help="Move the flow to a new directory and change the absolute paths."
    )
...
""",
    ...
    p_abiopen = subparsers.add_parser(
        "abiopen",
        parents=[copts_parser],  # , flow_selector_parser],
        help="Open FILE with abiopen. Same interface as abiopen.py",
    )
    p_abiopen.add_argument(
        "-nb", "--notebook", action="store_true", default=False, help="Open file in jupyter notebook"
    )
    p_abiopen.add_argument(
    ...
    p_abibuild = subparsers.add_parser(
        "abibuild", parents=[copts_parser, flow_selector_parser], help="Show ABINIT build information and exit."
    )

    # Subparser for doc_scheduler
    p_docsched = subparsers.add_parser(
        "doc_scheduler", parents=[copts_parser], help="Document the options available in scheduler.yml."
    )

    # Subparser for explain_(works|tasks)
    p_explain_works = subparsers.add_parser(
        "explain_works", parents=[copts_parser, flow_selector_parser], help="Explain operations performed by Works"
    )
    p_explain_tasks = subparsers.add_parser(
        "explain_tasks", parents=[copts_parser, flow_selector_parser], help="Explain operations performed by Tasks"
    )

    # Subparser for autoparal
    p_autoparal = subparsers.add_parser(
        "autoparal",
        parents=[copts_parser, flow_selector_parser],
    ...
    p_panel.add_argument(
        ...
        default=False,
        help=(
            "Start the bokeh server to serve the panel app "
    ...
    p_panel.add_argument(
        "--port", default=0, type=int, help="Allows specifying a specific port when serving panel app."
    )

    # Subparser for new_manager.
    p_new_manager = subparsers.add_parser(
        "new_manager", parents=[copts_parser, flow_selector_parser], help="Change the TaskManager."
    )
    ...
    # Subparser for tail.
    p_timeit = subparsers.add_parser(
        "timeit", parents=[copts_parser, flow_selector_parser], help="Extract timing data from Abinit output files."
    )
    ...
    p_robot.add_argument(
        ...
        default="GSR",
        help=(
            "The file extension of the output file (case insensitive). "
    ...
    p_cycles = subparsers.add_parser(
        ...
        parents=[copts_parser, flow_selector_parser],
        help=(
            "Print self-consistent/relaxation cycles extracted from the output of the tasks."
            "Use `-t ScfTask` or `FLOWDIR/w0/` or `FLOWDIR/w0/t1/` to select tasks by class, by work or by workdir."
        ),
    )
    p_cycles.add_argument(
        "-wok", "--exclude-ok-tasks", action="store_true", default=False, help="Exclude Tasks that have reached S_OK."
    )
    p_cycles.add_argument(
        "-p",
        "--plot-mode",
        ...
        help=(
            "Plot multiple cycles on the same figure if arg is specified. Use `-p` for gridplot. "
    ...
    p_inputs = subparsers.add_parser(
        "inputs", parents=[copts_parser, flow_selector_parser], help="Show the input files of the tasks."
    )
    ...
    p_structures = subparsers.add_parser(
        "structures",
        parents=[copts_parser, flow_selector_parser],
        help="Compare input/output structures of the tasks. Print max force and pressure if available.",
    )
    p_structures.add_argument(
        "--what", type=str, default="io", help="'i' for input structures, 'o' for output, 'io' for both."
    )

    # Subparser for ebands command.
    p_ebands = subparsers.add_parser(
        "ebands", parents=[copts_parser, flow_selector_parser], help="Compare electronic bands produced by the tasks."
    )
    p_ebands.add_argument(
        "-p",
        "--plot-mode",
        nargs="?",
        default=None,
    ...
    p_hist = subparsers.add_parser(
        "hist", parents=[copts_parser, flow_selector_parser], help="Compare HIST.nc files produced by the tasks."
    )
    p_hist.add_argument(
        "-p",
        "--plot-mode",
    ...
    p_corrections = subparsers.add_parser(
        "corrections", parents=[copts_parser, flow_selector_parser], help="Show AbiPy corrections performed at runtime."
    )

    # Subparser for history.
    p_history = subparsers.add_parser(
        "history", parents=[copts_parser, flow_selector_parser], help="Show Node history."
    )
    ...
    p_handlers = subparsers.add_parser(
        "handlers", parents=[copts_parser], help="Show event handlers installed in the flow."
    )
    p_handlers.add_argument(
        "-d",
        "--doc",
        action="store_true",
        default=False,
    ...
    p_notebook = subparsers.add_parser(
        "notebook", parents=[copts_parser], help="Create and open an ipython notebook to interact with the flow."
    )
    ...
    p_ipython = subparsers.add_parser(
        "ipython", parents=[copts_parser], help="Embed IPython. Useful for advanced operations or debugging purposes."
    )
    p_ipython.add_argument(
        "--argv",
        nargs="?",
        default="",
        type=shlex.split,
        help="Command-line options passed to ipython. Must be enclosed by quotes. Example: --argv='--matplotlib=wx'",
    )
    ...
    p_tricky = subparsers.add_parser(
        "tricky",
        parents=[copts_parser],
        help=(
            "Show tricky tasks i.e. tasks that have been restarted, "
    ...
    p_debug_reset = subparsers.add_parser(
        "debug_reset",
        parents=[copts_parser, flow_selector_parser],
    ...
    p_reset_jobids = subparsers.add_parser(
        "reset_jobids",
        parents=[copts_parser, flow_selector_parser],
        help="Analyze error files and log files produced by reset tasks for possible error messages.",
    )
    ...
    p_group = subparsers.add_parser(
        "group", parents=[copts_parser, flow_selector_parser], help="Group tasks according to property."
    )
    p_group.add_argument(
        ...
        default="status",
        type=str,
        choices=["status", "task_class"],
        help="Select the attribute used to group tasks. Default: status.",
    )

    # Subparser for diff.
    p_diff = subparsers.add_parser(
        "diff", parents=[copts_parser, flow_selector_parser], help="Compare files produced by two or three nodes."
    )
    p_diff.add_argument(
        "what_diff",
        nargs="?",
    ...
    p_networkx = subparsers.add_parser(
        "networkx", parents=[copts_parser], help="Draw flow and node dependencies with networkx package."
    )
    p_networkx.add_argument(
        "--nxmode",
        default="status",
    ...
    p_graphviz = subparsers.add_parser(
        ...
        parents=[copts_parser],
        help=(
            "Draw flow and node dependencies with graphviz package. Accept (FLOWDIR|WORKDIR|TASKDIR). "
    ...
    p_listext = subparsers.add_parser(
        "listext",
        parents=[copts_parser],
        help="List all the output files with the given extension that have been produced by the nodes.",
    )
    p_listext.add_argument(
        "listexts", nargs="*", default=[], help="List of Abinit file extensions. e.g DDB, GSR, WFK etc"
    )
...
def main():
    ...
    if not options.no_logo:
        nrows, ncols = get_terminal_size()
        if ncols > 100:
            cprint(abilab.abipy_logo1(), "yellow")

        system, node, release, version, machine, processor = platform.uname()
        cprint(
            "Running on %s -- system %s -- Python %s -- %s"
    ...
    if options.command in ("explain_works", "explain_tasks"):
        ...
        sched_options = {oname: getattr(options, oname) for oname in ("weeks", "days", "hours", "minutes", "seconds")}
        ...
        if options.task_status is None:
            options.task_status = Status.as_status("Running")

        paths = [
            get_path(task) for task in flow.iflat_tasks(status=options.task_status, nids=select_nids(flow, options))
        ]
        ...
        if not paths:
            ...
            cprint(
                "Press <CTRL+C> to interrupt. Number of output files %d\n" % len(paths),
                color="magenta",
                end="",
                flush=True,
            )
    ...
    #        print("qstats", task.qjob.get_stats())
        ...
        robot = abilab.Robot.from_flow(flow, outdirs="all", nids=select_nids(flow, options), ext=options.robot_ext)
        if len(robot) == 0:
        ...
        if options.print_dataframe:
            ...
            if hasattr(robot, "get_dataframe"):
                ...
                cprint(
                    "%s does not provide `get_dataframe` method. Using `to_string`" % (robot.__class__.__name__),
                    "yellow",
                )
                print(robot.to_string(verbose=options.verbose))
        ...
        cls2plotter = {}
        for task, cycle in flow.get_task_scfcycles(
            nids=select_nids(flow, options), exclude_ok_tasks=options.exclude_ok_tasks
        ):
            print()
            ...
            if pkey not in cls2plotter:
                cls2plotter[pkey] = CyclesPlotter()
        ...
        if options.plot_mode is not None and cls2plotter:
            for pkey, plotter in cls2plotter.items():
                try:
                    ...
                    cprint(
                        "Exception while invoking %s method of %s.\n%s"
                        % (options.plot_mode, plotter.__class__.__name__, str(exc)),
                        "red",
                    )

    elif options.command == "dims":
        flow.get_dims_dataframe(nids=select_nids(flow, options), printout=True, with_colors=not options.no_colors)
        ...
        flow.compare_abivars(
            varnames=options.varnames, nids=select_nids(flow, options), printout=True, with_colors=not options.no_colors
        )

    elif options.command == "structures":
        flow.compare_structures(
            nids=select_nids(flow, options),
            what=options.what,
            verbose=options.verbose,
            with_spglib=False,
