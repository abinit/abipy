...
class ConverterTest(AbipyTest):
    def test_ddb_phonopy_ddb_phfreqs(self):
        ...
        with DdbFile(os.path.join(test_dir, "AlAs_444_nobecs_DDB")) as ddb:
            ...
            phonon = ddb.anaget_phonopy_ifc(
                ngqpt=ngqpt, asr=0, dipdip=0, chneut=0, output_dir_path=tmp_dir, set_masses=True
            )

            orig_phbands = ddb.anaget_phmodes_at_qpoints(
                qpoints=qpoints, asr=0, dipdip=0, chneut=0, lo_to_splitting=False
            )
        ...
        scm = np.eye(3) * ngqpt
        ddb_conv = phonopy_to_abinit(
            unit_cell=structure,
            supercell_matrix=scm,
            qpt_list=qpoints.frac_coords,
            out_ddb_path=out_ddb_path,
            force_constants=os.path.join(tmp_dir, "FORCE_CONSTANTS"),
            born=None,
        ...
        conv_phbands = ddb_conv.anaget_phmodes_at_qpoints(
            qpoints=qpoints, asr=0, dipdip=0, chneut=0, lo_to_splitting=False
        )
    ...
    def test_ddb_phonopy_ddb_becs(self):
        ...
        with DdbFile(os.path.join(test_dir, "ZnO_gamma_becs_DDB")) as ddb:
            ...
            orig_phbands = ddb.anaget_phmodes_at_qpoints(
                qpoints=qpoints, asr=0, dipdip=0, chneut=0, lo_to_splitting=False, workdir=orig_run_ana
            )
        ...
        conv_phbands = ddb_conv.anaget_phmodes_at_qpoints(
            qpoints=qpoints, asr=0, dipdip=0, chneut=0, lo_to_splitting=False, workdir=conv_run_ana
        )
    ...
    def test_phonopy_ddb_phonopy(self):
        ...
        qpoints = ddb.qpoints
        phonon_conv = ddb.anaget_phonopy_ifc(
            ngqpt=ngqpt, asr=0, dipdip=0, chneut=0, output_dir_path=tmp_dir, set_masses=True
        )
    ...
    def test_tdep_ddb(self):
        ...
        phbands = ddb.anaget_phmodes_at_qpoints(qpoints=[[0, 0, 0]], asr=0, dipdip=0, chneut=0, lo_to_splitting=False)
