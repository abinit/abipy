...
class Qha2dTest(AbipyTest):
    def test_zsisa_approximation(self):
        ...
        dos_paths = [
            [os.path.join(root, f"scale_{s1}_{s3}/out_PHDOS.nc") for s3 in phdos_strains_c] for s1 in phdos_strains_a
        ]
    ...
    def test_qha_2d(self):
        ...
        gsr_paths = [[os.path.join(root, f"scale_{s1}_{s3}/out_GSR_DDB") for s3 in bo_strains_c] for s1 in bo_strains_a]
        phdos_paths = [
            [os.path.join(root, f"scale_{s1}_{s3}/out_PHDOS.nc") for s3 in bo_strains_c] for s1 in bo_strains_a
        ]
