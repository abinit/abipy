...
class ScfCycle(Mapping):
    ...
    def plotly(self, fontsize=12, **kwargs):
        """
        ...
        # Build fig.
        fig, go = get_figs_plotly(nrows=nrows, ncols=ncols, sharex=True, sharey=False)

        iter_num = np.array(list(range(self.num_iterations))) + 1

        for i, (key, values) in enumerate(self.items()):
            ...
            trace = go.Scatter(x=xx, y=yy, mode="lines+markers", showlegend=False, **kwargs)
            fig.add_trace(trace, row=row, col=col)
            fig.update_xaxes(title_text="Iteration Step", row=row, col=col)
            ...
            if key in _VARS_WITH_YRANGE:
                ...
                val_min, val_max = np.min(yy), np.max(yy)
                if abs(val_max - val_min) > abs(ymax - ymin):
                    plotly_set_lims(fig, (ymin, ymax), "y", iax=i + 1)
