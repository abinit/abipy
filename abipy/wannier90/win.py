...
class Wannier90Input(AbstractInput, Has_Structure):
    ...
    def from_abinit_file(cls, filepath: str) -> Wannier90Input:
        ...
        with abilab.abiopen(filepath) as abifile:
            ...
            new.set_vars(
                exclude_bands_="2, 6-8, 12", dis_win_min_=0.0, dis_win_max_=0.0, dis_froz_min_=0.0, dis_froz_max_=0.0
            )
            ...
            if ebands is not None:
                ...
                new.set_vars(num_bands=ebands.mband, mp_grid=mp_divs, _kpoints="?? ?? ??\n?? ?? ??")
                if ebands.nsppol == 2:
                    new.set_vars(spin="up")
    ...
    def set_kpath(self, bands_num_points=100, qptbounds=None):
        ...
        for i, kstart in enumerate(self.structure.hsym_kpoints[:-1]):
            ...
            app(
                "%s %.5f %.5f %.5f   %s %.5f %.5f %.5f"
                % (
                    kstart.name,
                    kstart.frac_coords[0],
                    kstart.frac_coords[1],
                    kstart.frac_coords[2],
                    kstop.name,
                    kstop.frac_coords[0],
                    kstop.frac_coords[1],
                    kstop.frac_coords[2],
