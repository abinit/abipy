...
class HasScfParams(AbipyModel):

    ##################
    # Input parameters
    ##################
    kppa: int = Field(1000, gt=0,
    ...
    #        if k not in abinit_variables:
    #            raise ValueError(f"{k} is not a valid Abinit input variable")
    #        if
    #    return values
...
class EbandsFlowModelWithInputs(_EbandsFlowModel):
    """
    More flexible FlowModel for band structure calculations that requires |AbinitInput| objects
    explicitly generated by the user before calling __init__.
    """

    @root_validator
    def check_inputs(cls, values):
        """Enforce the presence of scf_input and nscf_input once the model is instanciated."""
...
#class RelaxFlowModelWithParams(FlowModel):
#
...
#                                              mng_connector, with_gsr, with_hist)
