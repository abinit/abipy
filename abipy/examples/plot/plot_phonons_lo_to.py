#!/usr/bin/env python
"""
This example shows how to plot the phonon band structure of AlAs including the LO-TO splitting.
...
# Phonon frequencies with non analytical contributions, if calculated, are saved in the anaddb.nc
# file produced by anaddb. Those results should be fetched from there and added to the phonon bands.
phbands.read_non_anal_from_file(abidata.ref_file("ZnSe_hex_886.anaddb.nc"))

# Notice that all the directions starting from or arriving at gamma that are used in the path should
# explicitely calculated, even if the values are the same.
