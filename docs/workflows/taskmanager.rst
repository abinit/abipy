...
.. IMPORTANT::
    ...
    It's also a very good idea to run the Abinit test suite with the `runtests.py script <https://asciinema.org/a/40324>`_
...
.. note:
    ...
    This laptop has 1 socket with 2 CPUs and 4 Gb of memory in total, hence we don't want to run
...
(use ``abirun.py --help`` to get the list of possible commands and ``abirun.py COMMAND --help`` to document the
...
In the best case, the system administrator of our cluster already provides
...
Note that we are not specifying the number of cores in ``qparams`` because AbiPy will find
an appropriate value at run-time.
...
Note that there are more advanced options supported by ``limits`` and other options
...
to use the scheduler to automate job submissions.
...
This file is removed automatically when the scheduler completes its execution.
...
The AbiPy tasks are equipped with built-in event handlers that are be executed
